...
import {
  ArrowLeft, Camera, User, Mail, Phone, Shield, Clock, Package,
  CheckCircle2, ChevronDown, Lock,
} from 'lucide-react-native';
import { pharmacistService } from '@/services/pharmacistService';

/* ───── Data ───── */

const DEPARTMENTS = ['Pharmacy — Main Block', 'Pharmacy — Emergency', 'Pharmacy — OPD Wing', 'Central Drug Store'];
const SHIFTS = ['Morning (8 AM - 4 PM)', 'Afternoon (12 PM - 8 PM)', 'Night (8 PM - 8 AM)'];
...
interface FormField {
  ...
  iconColor: string;
  placeholder: string;
  type: 'text' | 'email' | 'phone' | 'select' | 'multiline' | 'readonly';
  options?: string[];
  section: 'personal' | 'work' | 'contact';
}

const FORM_FIELDS: FormField[] = [
  { key: 'fullName',  label: 'Full Name',            icon: User,    iconColor: Colors.primary, placeholder: '—', type: 'readonly', section: 'personal' },
  { key: 'email',     label: 'Email Address',         icon: Mail,    iconColor: '#0EA5E9',      placeholder: 'your@email.com', type: 'email', section: 'contact' },
  { key: 'phone',     label: 'Phone Number',          icon: Phone,   iconColor: '#059669',      placeholder: '—', type: 'readonly', section: 'contact' },
  { key: 'department',label: 'Department',            icon: Package, iconColor: '#8B5CF6',      placeholder: 'Select department', type: 'select', options: DEPARTMENTS, section: 'work' },
  { key: 'shift',     label: 'Preferred Shift',       icon: Clock,   iconColor: '#EA580C',      placeholder: 'Select shift', type: 'select', options: SHIFTS, section: 'work' },
  { key: 'license',   label: 'Pharmacy License No.',  icon: Shield,  iconColor: '#059669',      placeholder: 'e.g., PH-TN-2024-0042', type: 'text', section: 'work' },
];
  ...
  if (field.type === 'readonly') {
    return (
      <View className="mb-4">
        ...
        <View className="flex-row items-center bg-slate-50 rounded-2xl px-4 py-3.5 border border-slate-100">
          ...
          <Text className="flex-1 text-sm text-slate-500 font-medium">{value || '—'}</Text>
...
export default function PharmacistEditProfileScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const p = await pharmacistService.getProfile();
      ...
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Save', onPress: async () => {
          ...
          try {
            ...
            CustomAlert.alert('Error', 'Failed to save. Please try again.');
          } finally {
            setSaving(false);
          }
        },
  ...
  const completionPercent = useMemo(() => {
    const editable = ['email', 'department', 'shift', 'license'];
    const filled = editable.filter((k) => formData[k]?.trim().length > 0).length;
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#F8FAFC]" edges={['top']}>
      <View className="flex-row items-center px-4 pt-2 pb-2 bg-white border-b border-slate-100">
        ...
        <Pressable onPress={handleSave} disabled={!hasChanges || saving} className={`px-4 py-2 rounded-full ${hasChanges && !saving ? 'bg-primary' : 'bg-slate-200'}`}>
          {saving
            ? <ActivityIndicator size="small" color="#FFF" />
            : <Text className={`text-xs font-bold ${hasChanges && !saving ? 'text-white' : 'text-slate-400'}`}>Save</Text>
          }
        </Pressable>
      </View>
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1">
            <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 140 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              <View className="items-center pt-6 pb-4">
                ...
                <Text className="text-slate-400 text-xs mt-0.5">EMP: {empId || 'N/A'}</Text>
              </View>
