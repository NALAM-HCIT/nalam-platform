...
import {
  ArrowLeft, Camera, User, Mail, Phone, Clock,
...
export default function ReceptionistEditProfileScreen() {
  ...
  const [formData, setFormData] = useState<Record<string, string>>({
    fullName: userName || 'Receptionist',
  ...
  const initials = useMemo(() => {
    const parts = (formData.fullName || 'R').split(' ');
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1">
        <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 140 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View className="items-center pt-6 pb-4">
            ...
            <Text className="text-slate-400 text-xs mt-0.5">EMP: EMP-REC-042</Text>
          </View>
