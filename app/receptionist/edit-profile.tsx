...
import {
  ArrowLeft, Camera, User, Mail, Phone, Clock, Calendar,
...
const DEPARTMENTS = ['Front Office', 'Emergency Reception', 'OPD Reception', 'Billing Counter'];
...
const FORM_FIELDS: FormField[] = [
  ...
  { key: 'department', label: 'Department',      icon: Calendar, iconColor: '#8B5CF6',      placeholder: 'Select department', type: 'select', options: DEPARTMENTS, section: 'work' },
...
export default function ReceptionistEditProfileScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const p = await receptionistService.getProfile();
        setEmpId(p.employeeId ?? 'N/A');
        setFormData({
          fullName:   p.fullName,
          email:      p.email ?? '',
    ...
    CustomAlert.alert('Save Changes', 'Update your profile?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Save', onPress: async () => {
          setSaving(true);
          try {
            await receptionistService.updateProfile({
  ...
  const initials = useMemo(() => {
    const name = formData.fullName || 'R';
  ...
  const completionPercent = useMemo(() => {
    const editable = ['email', 'department', 'shift'];
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1">
            <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 140 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              <View className="items-center pt-6 pb-4">
                ...
                <Text className="text-slate-400 text-xs mt-0.5">EMP: {empId}</Text>
              </View>
